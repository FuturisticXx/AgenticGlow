...
final class HookNormalizerTests: XCTestCase {
    ...
    func testWorktreeSessionLeadsWithTheRepositoryThenTheWorktree() throws {
        for cwd in [
            "/workspace/test-project/.claude/worktrees/test-project-version-check-eaec6d",
            "/workspace/test-project/.claude/worktrees/test-project-version-check-eaec6d/src"
        ] {
            let event = try XCTUnwrap(HookNormalizer.normalize(
                provider: .claude,
                event: .userPromptSubmit,
            ...
            XCTAssertEqual(event.projectName, "test-project · version-check")
    ...
    func testWorktreeNamedOnlyForItsRepositoryDropsTheTrailingLabel() throws {
        let event = try XCTUnwrap(HookNormalizer.normalize(
            ...
            payload: [
                "session_id": "abc",
                "cwd": "/workspace/test-project/.claude/worktrees/test-project-abc123"
            ],
            environment: [:],
            processIdentity: .fixture,
            previous: nil,
        ...
        XCTAssertEqual(event.projectName, "test-project")
    }
    ...
    func testHandMadeWorktreeNameSurvivesIntact() throws {
        let event = try XCTUnwrap(HookNormalizer.normalize(
            ...
            payload: [
                "session_id": "abc",
                "cwd": "/workspace/test-project/.claude/worktrees/spike"
            ],
            environment: [:],
            processIdentity: .fixture,
            previous: nil,
        ...
        XCTAssertEqual(event.projectName, "test-project · spike")
    }
    ...
    func testOrdinaryDotClaudeDirectoryKeepsItsOwnBasename() throws {
        ...
        let event = try XCTUnwrap(HookNormalizer.normalize(
            provider: .claude,
            event: .userPromptSubmit,
            payload: ["session_id": "abc", "cwd": "/workspace/test-project/.claude/skills"],
            environment: [:],
            processIdentity: .fixture,
            previous: nil,
            now: Date(timeIntervalSince1970: 500)
