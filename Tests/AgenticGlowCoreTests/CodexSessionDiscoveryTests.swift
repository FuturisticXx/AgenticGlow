...
final class CodexSessionDiscoveryTests: XCTestCase {
    func testRecentThreadSurvivesDeletedWorkingDirectoryWithoutReadingContent() async throws {
        ...
        let response = Data(
            """
            {"id":8,"result":{"data":[{"id":"thread-1","name":"SECRET title","preview":"SECRET prompt","cwd":"/workspace/deleted-project","updatedAt":1784693299,"status":{"type":"notLoaded"},"source":"vscode"}],"nextCursor":null}}
            """.utf8
        )
        ...
        XCTAssertEqual(event.workingDirectory, "/workspace/deleted-project")
    ...
    func testExistingWorkingDirectoryUsesItsProjectName() async throws {
        let response = Data(
            """
            {"id":8,"result":{"data":[{"id":"thread-2","cwd":"/workspace/agenticglow","updatedAt":1784693299,"status":{"type":"idle"},"source":"cli"}],"nextCursor":null}}
            """.utf8
        )
        let adapter = CodexSessionDiscoveryAdapter(
            requester: StubCodexThreadRequester(data: response),
            workingDirectoryExists: { $0 == "/workspace/agenticglow" },
    ...
    func testDiscoveryReplacesStaleHookButPreservesClaude() {
        ...
        let claude = NormalizedEvent(
            ...
            workingDirectory: "/workspace/agenticglow",
