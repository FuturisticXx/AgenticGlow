...
final class WorkIdentityTests: XCTestCase {
    func testSamePathNormalizesToSameValue() {
        let a = WorkIdentity.normalize(path: "/workspace/agenticglow")
        let b = WorkIdentity.normalize(path: "/workspace/agenticglow")
        XCTAssertEqual(a?.value, b?.value)
        XCTAssertEqual(a?.rawPath, "/workspace/agenticglow")
    }
    ...
    func testDoesNotRequireTheFolderToExist() {
        let path = "/missing/deleted-project"
