...
final class WorkGroupingTests: XCTestCase {
    ...
    func testSamePathGroupsAcrossHarnesses() {
        let sessions = [
            session(id: "cursor", provider: .cursor, path: "/workspace/Caliber"),
            session(id: "claude", provider: .claude, path: "/workspace/Caliber/")
        ]
    ...
    func testDifferentPathsDoNotGroupEvenWithSameBasename() {
        let sessions = [
            session(id: "a", path: "/workspace/agenticglow"),
    ...
    func testWorktreeDoesNotMergeWithMainRepo() {
        let main = "/workspace/Caliber Wallet"
        let worktree = "\(main)/.git/worktrees/caliber-5-5a-composition-d702c0"
    ...
    func testCollidingBasenamesGetParentSuffixOnLaterWork() {
        let groups = WorkGrouping.groups(from: [
            session(id: "first", path: "/workspace/AgenticGlow"),
    ...
    func testSameSessionIDAndWorkCollapsesCursorAndClaudeHookReports() {
        ...
        let path = "/workspace/agenticglow"
        let claude = session(
            id: sid,
            provider: .claude,
            phase: .thinking,
