...
final class WorkDisplayNameTitleTests: XCTestCase {
    ...
    func testWorktreeSlugBecomesHumanTitle() {
        let path = "/workspace/Caliber Wallet/.claude/worktrees/goal-5-5c-execution-04a6d8"
    ...
    func testAmbiguousWorktreeSuffixesStayVisible() {
        let root = "/workspace/Caliber Wallet/.claude/worktrees"
    ...
    func testHumanFolderNamesStayExact() {
        let groups = WorkGrouping.groups(from: [
            session(id: "glow", path: "/workspace/AgenticGlow"),
            session(id: "caliber", path: "/workspace/Caliber Wallet")
        ])
        XCTAssertEqual(
