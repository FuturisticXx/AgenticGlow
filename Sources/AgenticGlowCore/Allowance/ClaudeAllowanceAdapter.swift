...
public struct ClaudeWebUsageClient: ClaudeUsageRequesting {
    ...
    public init() {
        self.session = CredentialUsageOriginBoundary.session()
    }
    ...
    public func fetchUsage(sessionCookie: String) async throws -> Data {
        ...
        guard let endpoint = request.url,
              CredentialUsageOriginBoundary.holds(response: response, endpoint: endpoint),
