...
Klarity is a free, open-source macOS menu bar utility that shows the live status of local Codex and Claude coding sessions.
...
Klarity is local-only. It does not require an account, operate a backend, collect analytics, upload prompts, or store model responses.
...
Support depends on the source application emitting its documented local lifecycle hooks. Klarity's diagnostics must report when an installed version does not expose the required events.

## Working Product Name

The working product name is **Klarity**.
...
Implementation may proceed under the Klarity working title. Public branding, release packaging, and external publication must stop if the clearance pass identifies unacceptable confusion or legal risk.
...
- Approving or denying permissions from Klarity.
...
Klarity uses a native SwiftUI and AppKit menu bar application, a small native event helper, provider-specific hook definitions, and local per-session state files.

### Components

#### 1. Klarity macOS application
...
#### 2. Klarity event helper

Klarity ships a small universal command-line executable at a stable user-level path under:

`~/Library/Application Support/Klarity/bin/`
...
The setup wizard merges Klarity handlers into Claude's supported hook configuration for:
...
The integration must preserve existing hooks and remove only entries carrying Klarity's unique marker.

#### 4. Codex integration

The setup wizard merges Klarity handlers into the user-level Codex hook configuration for:
...
Codex requires users to review and trust new or changed non-managed hooks. Klarity must explain this requirement and provide direct instructions to open Codex's `/hooks` interface. Klarity must not bypass hook trust.

Codex does not currently provide the same session-end event used by Claude. Klarity therefore uses source-process liveness and conservative expiration rules for Codex cleanup.

#### 5. Local state store

Normalized state files live under:

`~/Library/Application Support/Klarity/Sessions/`
...
Klarity stores no prompt text, assistant message, tool arguments, shell command, tool response, transcript contents, or file contents.
...
| Provider event | Klarity phase | Timer behavior |
| --- | --- | --- |
...
- Use a neutral Klarity symbol rather than OpenAI or Anthropic branding.
...
- Remain visible while Klarity is running, including when no session is active.
...
- If exact focusing is unavailable, Klarity brings the correct application to the foreground and clearly avoids claiming deeper navigation.

## Setup and Integration Management

### First-launch wizard

1. Explain what Klarity monitors and what it does not collect.
...
7. Confirm that Klarity receives and displays the test session.
...
Repair removes stale Klarity entries, installs the current entries, preserves unrelated configuration, and repeats the synthetic event test.

### Removal

Removal deletes only Klarity-marked hook entries and Klarity-owned local state. It does not restore an entire backup over newer user settings.
...
- Writes are atomic and constrained to Klarity-owned paths.
...
- MIT license for Klarity's original code.
- README credits the original MIT-licensed Claude Status Bar project as product inspiration while clearly stating that Klarity is an independent implementation.
...
- Klarity does not silently download or install updates.
...
6. Repair and removal touch only Klarity-owned entries and files.
7. Crashes, interruptions, denied permissions, and force quits do not leave permanent active indicators.
8. No prohibited content is written to Klarity's state or logs.
9. The app passes unit, integration, UI, accessibility, universal-build, signing, notarization, DMG, and Homebrew verification.
10. Public release is blocked until the Klarity name and distribution identifiers receive explicit clearance.
...
- Existing Klarity AI company: <https://www.klarity.ai/>
- Existing Klarity Disk macOS listing: <https://apps.apple.com/us/app/klarity-disk/id6758895498>
