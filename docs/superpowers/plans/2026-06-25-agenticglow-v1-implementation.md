# AgenticGlow v1 Implementation Plan
...
- Product working name: `AgenticGlow`.
...
- AgenticGlow may activate the correct source application, but must not claim exact terminal-tab or agent-thread navigation unless a stable supported mechanism exists.
...
- `Config/AgenticGlow-Info.plist`: App metadata, menu bar-only activation policy, version, and bundle identifier.
- `Config/AgenticGlow.entitlements`: Hardened runtime entitlement file with no App Sandbox entitlement.
- `Config/Debug.xcconfig`: Local debug defaults.
- `Config/Release.xcconfig`: Release build defaults.

### Shared core

- `Sources/AgenticGlowCore/Product/ProductMetadata.swift`: Stable product constants.
- `Sources/AgenticGlowCore/Events/AgentProvider.swift`: Provider and source-surface enums.
- `Sources/AgenticGlowCore/Events/NormalizedEvent.swift`: Versioned local event schema and validation.
- `Sources/AgenticGlowCore/Events/HookEventKind.swift`: Supported provider lifecycle event names.
- `Sources/AgenticGlowCore/Events/ToolCategory.swift`: Safe tool-name to display-label mapping.
- `Sources/AgenticGlowCore/Events/HookNormalizer.swift`: Claude and Codex payload normalization without persisting prohibited content.
- `Sources/AgenticGlowCore/State/SessionKey.swift`: Stable provider and session identity plus safe filename generation.
- `Sources/AgenticGlowCore/State/SessionStateStore.swift`: Atomic per-session JSON persistence and loading.
- `Sources/AgenticGlowCore/State/SessionSnapshot.swift`: Display-ready session state.
- `Sources/AgenticGlowCore/State/SessionResolver.swift`: Priority, sorting, completion duration, disconnection, and expiration rules.
- `Sources/AgenticGlowCore/Processes/ProcessIdentity.swift`: Process identity and source-app metadata.
- `Sources/AgenticGlowCore/Processes/ProcessInspector.swift`: Darwin process inspection boundary.
- `Sources/AgenticGlowCore/Processes/ProcessIdentityResolver.swift`: Provider ancestor and source bundle detection.
- `Sources/AgenticGlowCore/Processes/SourceApplicationActivator.swift`: Best-effort activation of the correct app.
- `Sources/AgenticGlowCore/Integrations/HookDefinitionFactory.swift`: Exact AgenticGlow hook entries for Claude and Codex.
- `Sources/AgenticGlowCore/Integrations/JSONConfigEditor.swift`: Validated backup, merge, atomic write, and ownership checks.
- `Sources/AgenticGlowCore/Integrations/ClaudeIntegrationManager.swift`: Claude settings install, diagnosis, repair, and removal.
- `Sources/AgenticGlowCore/Integrations/CodexIntegrationManager.swift`: Codex hooks install, diagnosis, repair, removal, and trust instructions.
- `Sources/AgenticGlowCore/Integrations/HelperInstaller.swift`: Copies and verifies the embedded helper at its stable path.
- `Sources/AgenticGlowCore/Updates/UpdateChecker.swift`: Opt-in GitHub release check.
- `Sources/AgenticGlowCore/Logging/DiagnosticLogger.swift`: Disabled-by-default sanitized local diagnostics.

### Event helper

- `Sources/AgenticGlowCore/Helper/AgenticGlowEventCommand.swift`: Testable CLI orchestration shared with the helper target.
- `Sources/AgenticGlowEvent/main.swift`: Reads arguments and standard input, then exits with a stable code.

### App

- `Sources/AgenticGlowApp/AgenticGlowApp.swift`: SwiftUI app entry point.
- `Sources/AgenticGlowApp/AppDelegate.swift`: Menu bar lifecycle and settings-window coordination.
- `Sources/AgenticGlowApp/AppModel.swift`: Main-actor observable state and 0.5-second refresh loop.
- `Sources/AgenticGlowApp/MenuBar/StatusItemController.swift`: `NSStatusItem`, popover, title, animation, and accessibility.
- `Sources/AgenticGlowApp/MenuBar/StatusPresentation.swift`: Pure mapping from resolved state to icon, label, count, and color.
- `Sources/AgenticGlowApp/MenuBar/SessionListView.swift`: Provider-grouped session list and footer actions.
- `Sources/AgenticGlowApp/MenuBar/SessionRowView.swift`: Project, source, status, and elapsed time.
- `Sources/AgenticGlowApp/Setup/SetupView.swift`: First-launch privacy explanation and integration setup.
- `Sources/AgenticGlowApp/Setup/SetupViewModel.swift`: Detection, install, synthetic test, trust, and repair state.
- `Sources/AgenticGlowApp/Settings/SettingsView.swift`: Timer, animation, launch-at-login, updates, and integrations.
- `Sources/AgenticGlowApp/Services/LaunchAtLoginService.swift`: `SMAppService.mainApp` wrapper.
- `Sources/AgenticGlowApp/Services/SyntheticEventService.swift`: Local setup verification through the installed helper.
- `Sources/AgenticGlowApp/Resources/Assets.xcassets`: App icon and color assets.

### Tests and fixtures

- `Tests/AgenticGlowCoreTests/`: Unit tests for schema, normalization, state, process logic, configuration, updates, and logging.
- `Tests/AgenticGlowEventTests/`: Command orchestration tests using temporary directories and injected dependencies.
- `Tests/AgenticGlowAppTests/`: App-model and presentation tests.
- `Tests/AgenticGlowUITests/`: First launch, setup, menu bar, permission, multi-session, repair, and accessibility smoke tests.
...
- `Design/AgenticGlowIcon-1024.png`: Approved source icon.
...
- `Cask/agenticglow.rb`: Generated Homebrew Cask for the current release.
...
**Files:**
- Create: `project.yml`
- Create: `Config/AgenticGlow-Info.plist`
- Create: `Config/AgenticGlow.entitlements`
- Create: `Config/Debug.xcconfig`
- Create: `Config/Release.xcconfig`
- Create: `Sources/AgenticGlowCore/Product/ProductMetadata.swift`
- Create: `Sources/AgenticGlowEvent/main.swift`
- Create: `Sources/AgenticGlowApp/AgenticGlowApp.swift`
- Create: `Sources/AgenticGlowApp/AppDelegate.swift`
- Create: `Sources/AgenticGlowApp/Resources/Assets.xcassets/Contents.json`
- Create: `Tests/AgenticGlowCoreTests/ProductMetadataTests.swift`
- Create: `Tests/AgenticGlowEventTests/AgenticGlowEventSmokeTests.swift`
- Create: `Tests/AgenticGlowAppTests/AgenticGlowAppSmokeTests.swift`
- Create: `Tests/AgenticGlowUITests/AgenticGlowLaunchTests.swift`
- Create: `Tests/TestSupport/TestSupport.swift`

**Interfaces:**
- Produces: `ProductMetadata.displayName`, `ProductMetadata.bundleIdentifier`, `ProductMetadata.schemaVersion`, and generated schemes `AgenticGlow`, `AgenticGlowEvent`, and `AgenticGlowCore`.
- Consumes: Nothing.
...
targets:
  AgenticGlowCore:
    ...
    sources:
      - Sources/AgenticGlowCore
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.twodamax.agenticglow.core
        DEFINES_MODULE: YES
  AgenticGlowEvent:
    type: tool
    platform: macOS
    deploymentTarget: "14.0"
    sources:
      - Sources/AgenticGlowEvent
    dependencies:
      - target: AgenticGlowCore
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.twodamax.agenticglow.event
        PRODUCT_NAME: agenticglow-event
  AgenticGlow:
    type: application
    platform: macOS
    deploymentTarget: "14.0"
    sources:
      - Sources/AgenticGlowApp
    resources:
      - Sources/AgenticGlowApp/Resources
    info:
      path: Config/AgenticGlow-Info.plist
    entitlements:
      path: Config/AgenticGlow.entitlements
    dependencies:
      - target: AgenticGlowCore
      - target: AgenticGlowEvent
        embed: false
    postBuildScripts:
      - name: Embed AgenticGlow event helper
        basedOnDependencyAnalysis: false
        script: |
          set -euo pipefail
          destination="$TARGET_BUILD_DIR/$UNLOCALIZED_RESOURCES_FOLDER_PATH/bin"
          mkdir -p "$destination"
          cp "$BUILT_PRODUCTS_DIR/agenticglow-event" "$destination/agenticglow-event"
          chmod 755 "$destination/agenticglow-event"
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.twodamax.agenticglow
        PRODUCT_NAME: AgenticGlow
        GENERATE_INFOPLIST_FILE: NO
  AgenticGlowCoreTests:
    type: bundle.unit-test
    platform: macOS
    deploymentTarget: "14.0"
    sources:
      - Tests/AgenticGlowCoreTests
      - Tests/TestSupport
      - Tests/Fixtures
    dependencies:
      - target: AgenticGlowCore
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.twodamax.agenticglow.core-tests
  AgenticGlowEventTests:
    type: bundle.unit-test
    platform: macOS
    deploymentTarget: "14.0"
    sources:
      - Tests/AgenticGlowEventTests
      - Tests/TestSupport
      - Tests/Fixtures
    dependencies:
      - target: AgenticGlowCore
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.twodamax.agenticglow.event-tests
  AgenticGlowAppTests:
    type: bundle.unit-test
    platform: macOS
    deploymentTarget: "14.0"
    sources:
      - Tests/AgenticGlowAppTests
      - Tests/TestSupport
    dependencies:
      - target: AgenticGlow
      - target: AgenticGlowCore
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.twodamax.agenticglow.app-tests
  AgenticGlowUITests:
    type: bundle.ui-testing
    platform: macOS
    deploymentTarget: "14.0"
    sources:
      - Tests/AgenticGlowUITests
    dependencies:
      - target: AgenticGlow
    settings:
      base:
        PRODUCT_BUNDLE_IDENTIFIER: com.twodamax.agenticglow.ui-tests
schemes:
  AgenticGlow:
    build:
      targets:
        AgenticGlow: all
        AgenticGlowCoreTests: [test]
        AgenticGlowEventTests: [test]
        AgenticGlowAppTests: [test]
        AgenticGlowUITests: [test]
    test:
      targets:
        - AgenticGlowCoreTests
        - AgenticGlowEventTests
...
```swift
// Sources/AgenticGlowCore/Product/ProductMetadata.swift
import Foundation

public enum ProductMetadata {
    public static let displayName = "AgenticGlow"
    public static let bundleIdentifier = "com.twodamax.agenticglow"
    public static let helperName = "agenticglow-event"
    public static let schemaVersion = 1
}
```

```swift
// Sources/AgenticGlowEvent/main.swift
import Foundation
import AgenticGlowCore

FileHandle.standardError.write(Data("agenticglow-event is not configured yet\n".utf8))
exit(64)
```

```swift
// Sources/AgenticGlowApp/AgenticGlowApp.swift
import SwiftUI
...
```swift
// Sources/AgenticGlowApp/AppDelegate.swift
import AppKit
...
```json
// Sources/AgenticGlowApp/Resources/Assets.xcassets/Contents.json
{
  "info": {
    "author": "xcode",
    "version": 1
...
```swift
// Tests/AgenticGlowCoreTests/ProductMetadataTests.swift
import XCTest
...
```swift
// Tests/AgenticGlowEventTests/AgenticGlowEventSmokeTests.swift
import XCTest
@testable import AgenticGlowCore
...
```swift
// Tests/AgenticGlowAppTests/AgenticGlowAppSmokeTests.swift
import XCTest
@testable import AgenticGlowCore
...
```swift
// Tests/AgenticGlowUITests/AgenticGlowLaunchTests.swift
import XCTest
...
```xml
<!-- Config/AgenticGlow-Info.plist -->
<?xml version="1.0" encoding="UTF-8"?>
<!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd">
<plist version="1.0">
<dict>
  <key>CFBundleDisplayName</key><string>AgenticGlow</string>
  ...
  <key>NSHumanReadableCopyright</key><string>Copyright © 2026 AgenticGlow contributors</string>
</dict>
</plist>
```
...
xcodebuild build \
  ...
  -destination 'platform=macOS' \
  CODE_SIGNING_ALLOWED=NO
test -x "$HOME/Library/Developer/Xcode/DerivedData/AgenticGlow-"*/Build/Products/Debug/AgenticGlow.app/Contents/Resources/bin/agenticglow-event
```
...
```bash
git add project.yml Config Sources Tests AgenticGlow.xcodeproj
git commit -m "build: scaffold AgenticGlow macOS targets"
```
...
**Files:**
- Create: `Sources/AgenticGlowCore/Events/AgentProvider.swift`
- Create: `Sources/AgenticGlowCore/Events/HookEventKind.swift`
- Create: `Sources/AgenticGlowCore/Events/ToolCategory.swift`
- Create: `Sources/AgenticGlowCore/Events/NormalizedEvent.swift`
- Create: `Tests/AgenticGlowCoreTests/NormalizedEventTests.swift`

**Interfaces:**
- Produces: `AgentProvider`, `SourceSurface`, `SessionPhase`, `HookEventKind`, `ToolCategory`, `NormalizedEvent`, `NormalizedEvent.validate()`, `JSONEncoder.agenticglow`, and `JSONDecoder.agenticglow`.
- Consumes: `ProductMetadata.schemaVersion`.

- [ ] **Step 1: Write failing schema tests**

```swift
// Tests/AgenticGlowCoreTests/NormalizedEventTests.swift
import XCTest
@testable import AgenticGlowCore
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/NormalizedEventTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```swift
// Sources/AgenticGlowCore/Events/AgentProvider.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Events/HookEventKind.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Events/ToolCategory.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Events/NormalizedEvent.swift
import Foundation
...
xcodebuild test \
  ...
  -skip-testing:AgenticGlowUITests \
  CODE_SIGNING_ALLOWED=NO
git add Sources/AgenticGlowCore/Events Tests/AgenticGlowCoreTests/NormalizedEventTests.swift
...
**Files:**
- Create: `Sources/AgenticGlowCore/Events/HookNormalizer.swift`
- Create: `Tests/AgenticGlowCoreTests/HookNormalizerTests.swift`
...
```swift
// Tests/AgenticGlowCoreTests/HookNormalizerTests.swift
import XCTest
@testable import AgenticGlowCore
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/HookNormalizerTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```swift
// Sources/AgenticGlowCore/Processes/ProcessIdentity.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Events/HookNormalizer.swift
import Foundation
...
```bash
git add Sources/AgenticGlowCore/Events Sources/AgenticGlowCore/Processes Tests
...
**Files:**
- Create: `Sources/AgenticGlowCore/State/SessionKey.swift`
- Create: `Sources/AgenticGlowCore/State/SessionStateStore.swift`
- Create: `Sources/AgenticGlowCore/Processes/ProcessIdentityResolver.swift`
- Create: `Sources/AgenticGlowCore/Helper/AgenticGlowEventCommand.swift`
- Modify: `Sources/AgenticGlowEvent/main.swift`
- Create: `Tests/AgenticGlowCoreTests/SessionStateStoreTests.swift`
- Create: `Tests/AgenticGlowEventTests/AgenticGlowEventCommandTests.swift`

**Interfaces:**
- Produces: `SessionKey`, `SessionStateStoring`, `FileSessionStateStore`, `AgenticGlowEventCommand.run(arguments:input:environment:now:) -> Int32`.
...
```swift
// Tests/AgenticGlowCoreTests/SessionStateStoreTests.swift
import XCTest
@testable import AgenticGlowCore
...
```swift
// Tests/AgenticGlowEventTests/AgenticGlowEventCommandTests.swift
import XCTest
@testable import AgenticGlowCore
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/SessionStateStoreTests \
  -only-testing:AgenticGlowEventTests/AgenticGlowEventCommandTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```swift
// Sources/AgenticGlowCore/State/SessionKey.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/State/SessionStateStore.swift
import Darwin
import Foundation
...
public final class FileSessionStateStore: SessionStateStoring {
    ...
    public static var defaultDirectory: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent("Library/Application Support/AgenticGlow/Sessions", isDirectory: true)
    }
    ...
    public func loadAll() throws -> [NormalizedEvent] {
        ...
        .compactMap { url in
            do {
                ...
                let event = try JSONDecoder.agenticglow.decode(
    ...
    public func load(_ key: SessionKey) throws -> NormalizedEvent? {
        ...
        try rejectSymlink(url)
        let event = try JSONDecoder.agenticglow.decode(NormalizedEvent.self, from: Data(contentsOf: url))
        try event.validate()
        return event
    }
...
```swift
// Sources/AgenticGlowCore/Helper/AgenticGlowEventCommand.swift
import Foundation
import AgenticGlowCore
...
```swift
// Sources/AgenticGlowCore/Processes/ProcessIdentityResolver.swift
import Foundation
...
AGENTICGLOW_STATE_DIRECTORY="$state_directory" \
  "$HOME/Library/Developer/Xcode/DerivedData/AgenticGlow-"*/Build/Products/Debug/agenticglow-event \
  codex UserPromptSubmit --agenticglow-hook \
  < Tests/Fixtures/codex/stop.json
...
```bash
git add Sources/AgenticGlowCore/State Sources/AgenticGlowEvent Tests
...
**Files:**
- Create: `Sources/AgenticGlowCore/State/SessionSnapshot.swift`
- Create: `Sources/AgenticGlowCore/State/SessionResolver.swift`
- Create: `Tests/AgenticGlowCoreTests/SessionResolverTests.swift`
...
```swift
// Tests/AgenticGlowCoreTests/SessionResolverTests.swift
import XCTest
@testable import AgenticGlowCore
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/SessionResolverTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```swift
// Sources/AgenticGlowCore/State/SessionSnapshot.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/State/SessionResolver.swift
import Foundation
...
```bash
git add Sources/AgenticGlowCore/State Tests/AgenticGlowCoreTests/SessionResolverTests.swift
...
**Files:**
- Modify: `Sources/AgenticGlowCore/Processes/ProcessIdentity.swift`
- Create: `Sources/AgenticGlowCore/Processes/ProcessInspector.swift`
- Modify: `Sources/AgenticGlowCore/Processes/ProcessIdentityResolver.swift`
- Create: `Sources/AgenticGlowCore/Processes/SourceApplicationActivator.swift`
- Create: `Tests/AgenticGlowCoreTests/ProcessIdentityResolverTests.swift`
...
```swift
// Tests/AgenticGlowCoreTests/ProcessIdentityResolverTests.swift
import XCTest
@testable import AgenticGlowCore
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/ProcessIdentityResolverTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```swift
// Sources/AgenticGlowCore/Processes/ProcessInspector.swift
import AppKit
import Darwin
import Foundation
...
```swift
// Sources/AgenticGlowCore/Processes/SourceApplicationActivator.swift
import AppKit
import Darwin
import Foundation
...
xcodebuild test \
  ...
  -skip-testing:AgenticGlowUITests \
  CODE_SIGNING_ALLOWED=NO
git add Sources/AgenticGlowCore/Processes Tests/AgenticGlowCoreTests
...
**Files:**
- Create: `Sources/AgenticGlowApp/AppModel.swift`
- Create: `Sources/AgenticGlowApp/MenuBar/StatusPresentation.swift`
- Create: `Sources/AgenticGlowApp/MenuBar/StatusItemController.swift`
- Create: `Sources/AgenticGlowApp/MenuBar/SessionListView.swift`
- Create: `Sources/AgenticGlowApp/MenuBar/SessionRowView.swift`
- Modify: `Sources/AgenticGlowApp/AppDelegate.swift`
- Create: `Tests/AgenticGlowAppTests/AppModelTests.swift`
- Create: `Tests/AgenticGlowAppTests/StatusPresentationTests.swift`
...
```swift
// Tests/AgenticGlowAppTests/AppModelTests.swift
import XCTest
...
```swift
// Tests/AgenticGlowAppTests/StatusPresentationTests.swift
import XCTest
@testable import AgenticGlow
@testable import AgenticGlowCore
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowAppTests/AppModelTests \
...
```swift
// Sources/AgenticGlowApp/AppModel.swift
import Foundation
import Observation
import AgenticGlowCore
...
```swift
// Sources/AgenticGlowApp/MenuBar/StatusPresentation.swift
import AppKit
import AgenticGlowCore
...
struct StatusPresentation: Equatable {
    ...
    init(resolved: ResolvedSessions, showTimer: Bool, reduceMotion: Bool) {
        ...
        case .permission:
            ...
            accessibilityLabel = "AgenticGlow, \(resolved.permissionCount) sessions need permission"
            color = .systemYellow
            animates = false
        case .usingTool, .thinking:
            ...
            accessibilityLabel = "AgenticGlow, \(resolved.activeCount) active sessions"
...
```swift
// Sources/AgenticGlowApp/MenuBar/StatusItemController.swift
import AppKit
import SwiftUI
import AgenticGlowCore
...
final class StatusItemController: NSObject {
    ...
    init(model: AppModel, openIntegrations: @escaping () -> Void) {
        ...
        item.button?.setAccessibilityLabel("AgenticGlow")
        update()
    }
...
```swift
// Sources/AgenticGlowApp/MenuBar/SessionListView.swift
import SwiftUI
import AgenticGlowCore
...
```swift
// Sources/AgenticGlowApp/MenuBar/SessionRowView.swift
import SwiftUI
import AgenticGlowCore
...
xcodebuild build \
  ...
  -configuration Debug \
  -destination 'platform=macOS'
open "$HOME/Library/Developer/Xcode/DerivedData/AgenticGlow-"*/Build/Products/Debug/AgenticGlow.app
```

Expected: tests PASS, AgenticGlow launches without a Dock icon, and one idle status item appears.

- [ ] **Step 8: Commit**

```bash
git add Sources/AgenticGlowApp Tests/AgenticGlowAppTests
...
**Files:**
- Create: `Sources/AgenticGlowCore/Integrations/HookDefinitionFactory.swift`
- Create: `Sources/AgenticGlowCore/Integrations/JSONConfigEditor.swift`
- Create: `Sources/AgenticGlowCore/Integrations/IntegrationStatus.swift`
- Create: `Sources/AgenticGlowCore/Integrations/ClaudeIntegrationManager.swift`
- Create: `Sources/AgenticGlowCore/Integrations/CodexIntegrationManager.swift`
- Create: `Tests/AgenticGlowCoreTests/JSONConfigEditorTests.swift`
- Create: `Tests/AgenticGlowCoreTests/ClaudeIntegrationManagerTests.swift`
- Create: `Tests/AgenticGlowCoreTests/CodexIntegrationManagerTests.swift`
...
```swift
// Tests/AgenticGlowCoreTests/ClaudeIntegrationManagerTests.swift
import XCTest
@testable import AgenticGlowCore
...
```swift
// Tests/AgenticGlowCoreTests/CodexIntegrationManagerTests.swift
import XCTest
@testable import AgenticGlowCore
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/ClaudeIntegrationManagerTests \
  -only-testing:AgenticGlowCoreTests/CodexIntegrationManagerTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```swift
// Sources/AgenticGlowCore/Integrations/IntegrationStatus.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Integrations/HookDefinitionFactory.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Integrations/JSONConfigEditor.swift
import Darwin
import Foundation
...
public final class JSONConfigEditor {
    ...
    private func writeFirstBackupIfNeeded(_ data: Data) throws {
        ...
        let backup = directory.appendingPathComponent(
            "\(prefix)\(formatter.string(from: Date())).bak-agenticglow"
        )
...
```swift
// Sources/AgenticGlowCore/Integrations/ClaudeIntegrationManager.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Integrations/CodexIntegrationManager.swift
import Foundation

public final class CodexIntegrationManager: ProviderIntegrationManaging {
    public static let trustInstruction =
        "Open Codex, run /hooks, review the AgenticGlow entries, and choose Trust."
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/JSONConfigEditorTests \
  -only-testing:AgenticGlowCoreTests/ClaudeIntegrationManagerTests \
  -only-testing:AgenticGlowCoreTests/CodexIntegrationManagerTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```bash
git add Sources/AgenticGlowCore/Integrations Tests/AgenticGlowCoreTests
...
**Files:**
- Create: `Sources/AgenticGlowCore/Integrations/HelperInstaller.swift`
- Create: `Sources/AgenticGlowCore/Integrations/ExecutableLocator.swift`
- Create: `Sources/AgenticGlowCore/Integrations/ProviderVersionDetector.swift`
- Create: `Sources/AgenticGlowApp/Setup/SetupViewModel.swift`
- Create: `Sources/AgenticGlowApp/Setup/SetupView.swift`
- Create: `Sources/AgenticGlowApp/Services/SyntheticEventService.swift`
- Create: `Sources/AgenticGlowApp/Services/LaunchAtLoginService.swift`
- Modify: `Sources/AgenticGlowApp/AgenticGlowApp.swift`
- Modify: `Sources/AgenticGlowApp/AppDelegate.swift`
- Create: `Tests/AgenticGlowCoreTests/HelperInstallerTests.swift`
- Create: `Tests/AgenticGlowAppTests/SetupViewModelTests.swift`
...
- Consumes: provider integration managers, embedded `agenticglow-event`, and `FileSessionStateStore`.

- [x] **Step 1: Write failing helper-installation and setup tests**

```swift
// Tests/AgenticGlowCoreTests/HelperInstallerTests.swift
import XCTest
@testable import AgenticGlowCore
...
```swift
// Tests/AgenticGlowAppTests/SetupViewModelTests.swift
import XCTest
@testable import AgenticGlow
@testable import AgenticGlowCore
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/HelperInstallerTests \
  -only-testing:AgenticGlowAppTests/SetupViewModelTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```swift
// Sources/AgenticGlowCore/Integrations/HelperInstaller.swift
import Foundation
...
public final class HelperInstaller: HelperInstalling {
    ...
    public func install() throws {
        ...
        try fileManager.createDirectory(at: directory, withIntermediateDirectories: true)
        try fileManager.setAttributes([.posixPermissions: 0o700], ofItemAtPath: directory.path)
        let temporary = directory.appendingPathComponent(".agenticglow-event.\(UUID().uuidString)")
...
```swift
// Sources/AgenticGlowCore/Integrations/ExecutableLocator.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Integrations/ProviderVersionDetector.swift
import Foundation
...
```swift
// Sources/AgenticGlowApp/Services/SyntheticEventService.swift
import Foundation
import AgenticGlowCore
...
struct SyntheticEventService: SyntheticEventTesting {
    ...
    func run(provider: AgentProvider, helperURL: URL) throws -> Bool {
        let sessionID = "agenticglow-setup-\(UUID().uuidString)"
        ...
        process.arguments = [provider.rawValue, HookEventKind.userPromptSubmit.rawValue, "--agenticglow-hook"]
...
```swift
// Sources/AgenticGlowApp/Setup/SetupViewModel.swift
import Foundation
import Observation
import AgenticGlowCore
...
```swift
// Sources/AgenticGlowApp/Setup/SetupView.swift
import SwiftUI
import AgenticGlowCore
...
```swift
// Sources/AgenticGlowApp/Services/LaunchAtLoginService.swift
...
This mode removes only AgenticGlow-marked hooks and AgenticGlow-owned helper files.
...
```bash
defaults delete com.twodamax.agenticglow 2>/dev/null || true
open "$HOME/Library/Developer/Xcode/DerivedData/AgenticGlow-"*/Build/Products/Debug/AgenticGlow.app
```
...
```bash
git add Sources/AgenticGlowCore/Integrations Sources/AgenticGlowApp Tests
...
**Files:**
- Create: `Sources/AgenticGlowCore/Updates/UpdateChecker.swift`
- Create: `Sources/AgenticGlowCore/Logging/DiagnosticLogger.swift`
- Create: `Sources/AgenticGlowApp/Settings/SettingsView.swift`
- Create: `Sources/AgenticGlowApp/Settings/PreferencesStore.swift`
- Create: `Sources/AgenticGlowApp/Settings/UpdateViewModel.swift`
...
- Create: `Sources/AgenticGlowApp/UITesting/UITestSessionStore.swift`
- Create: `Design/AgenticGlowIcon-1024.png`
- Create: `Sources/AgenticGlowApp/Resources/Assets.xcassets/AppIcon.appiconset/Contents.json`
- Create: `Tests/AgenticGlowCoreTests/UpdateCheckerTests.swift`
- Create: `Tests/AgenticGlowCoreTests/DiagnosticLoggerTests.swift`
- Create: `Tests/AgenticGlowUITests/AgenticGlowUITests.swift`
...
```swift
// Tests/AgenticGlowCoreTests/UpdateCheckerTests.swift
import XCTest
@testable import AgenticGlowCore
...
```swift
// Tests/AgenticGlowCoreTests/DiagnosticLoggerTests.swift
import XCTest
@testable import AgenticGlowCore

final class DiagnosticLoggerTests: XCTestCase {
    func testLoggerWritesOnlyApprovedMetadataWhenEnabled() throws {
        let url = temporaryDirectory().appendingPathComponent("agenticglow.log")
...
xcodebuild test \
  ...
  -scheme AgenticGlow \
  -destination 'platform=macOS' \
  -only-testing:AgenticGlowCoreTests/UpdateCheckerTests \
  -only-testing:AgenticGlowCoreTests/DiagnosticLoggerTests \
  CODE_SIGNING_ALLOWED=NO
```
...
```swift
// Sources/AgenticGlowCore/Updates/UpdateChecker.swift
import Foundation
...
```swift
// Sources/AgenticGlowCore/Logging/DiagnosticLogger.swift
import Foundation
...
Modify `AgenticGlowEventCommand` to accept:
...
Pass `logger` into `AgenticGlowEventCommand`.

```swift
// Sources/AgenticGlowApp/Settings/PreferencesStore.swift
import Foundation
import Observation
...
```swift
// Sources/AgenticGlowApp/Settings/UpdateViewModel.swift
import AppKit
import Foundation
import Observation
import AgenticGlowCore
...
```swift
// Sources/AgenticGlowApp/Settings/SettingsView.swift
import SwiftUI
...
```text
Create a 1024 by 1024 macOS application icon for AgenticGlow, a local AI coding-agent status monitor. Use an original neutral radial signal symbol formed from one clear central point and four balanced outward rays. Avoid OpenAI, Anthropic, Claude, Codex, chatbot, robot, brain, letter K, and copied brand shapes. Use a deep graphite background with a restrained cyan-to-mint signal glow, strong silhouette, large simple geometry, no text, no transparency, and enough contrast to remain recognizable at 16 pixels. Modern native macOS utility quality.
```

Save the approved image at `Design/AgenticGlowIcon-1024.png`. Generate the `.appiconset` sizes with `sips`, write a complete `Contents.json`, and add `ASSETCATALOG_COMPILER_APPICON_NAME = AppIcon` to `project.yml`.

Verify:

```bash
test "$(sips -g pixelWidth Design/AgenticGlowIcon-1024.png | awk '/pixelWidth/ {print $2}')" = "1024"
test "$(sips -g pixelHeight Design/AgenticGlowIcon-1024.png | awk '/pixelHeight/ {print $2}')" = "1024"
```
...
```swift
// Tests/AgenticGlowUITests/AgenticGlowUITests.swift
import XCTest
...
final class AgenticGlowUITests: XCTestCase {
    ...
    func testEmptyStateExplainsHowToStart() {
        ...
        app.launch()
        app.statusItems["AgenticGlow.StatusItem"].click()
...
```text
Copyright (c) 2026 AgenticGlow contributors
```
...
- Create: `Cask/agenticglow.rb`
- Create: `.github/workflows/release.yml`

**Interfaces:**
- Produces: `build/AgenticGlow.app`, `build/AgenticGlow-<version>.dmg`, verified signatures, notarization tickets, and a generated Homebrew Cask.
- Consumes: `DEVELOPER_ID_APPLICATION`, `NOTARY_PROFILE`, `AGENTICGLOW_NAME_CLEARED=1`, and `AGENTICGLOW_RELEASE_BUILD_APPROVED=1`.
...
Expected: syntax checks pass and the release-gate command FAILS with `Missing release gate: AGENTICGLOW_NAME_CLEARED`.
...
jobs:
  package:
    if: ${{ vars.AGENTICGLOW_PUBLICATION_APPROVED == '1' }}
    runs-on: macos-15
    steps:
      - uses: actions/checkout@v4
      - run: brew install xcodegen
...
The repository variables `AGENTICGLOW_PUBLICATION_APPROVED`, `AGENTICGLOW_NAME_CLEARED`, and `AGENTICGLOW_RELEASE_BUILD_APPROVED` must remain unset until explicit publication approval, name clearance, signing-secret configuration, and review of the exact release workflow.
...
brew install --cask ./Cask/agenticglow.rb
open /Applications/AgenticGlow.app
brew uninstall --cask agenticglow
```
...
helper="$HOME/Library/Developer/Xcode/DerivedData/AgenticGlow-"*/Build/Products/Debug/agenticglow-event
tmp_root="$(mktemp -d)"
...
run_fixture() {
  ...
  local fixture="$3"
  AGENTICGLOW_STATE_DIRECTORY="$state_directory" \
    "$helper" "$provider" "$event" --agenticglow-hook < "$fixture"
}
...
1. Install AgenticGlow's Claude integration.
2. Trust AgenticGlow's Codex hooks through `/hooks`.
...
git commit -m "test: verify AgenticGlow v0.1.0 release candidate"
```
