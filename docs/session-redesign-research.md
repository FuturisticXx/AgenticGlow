...
- **Reduce Motion bug:** `model.reduceMotion` is read once at `AppModel.init` from `NSWorkspace.shared.accessibilityDisplayShouldReduceMotion` and never re-observed. A user who toggles Reduce Motion in System Settings while AgenticGlow is running keeps seeing full motion until they restart the app. Small fix, real accessibility bug.
